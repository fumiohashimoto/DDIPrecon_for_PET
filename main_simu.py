...
from solver_pet_ddiprecon import Diffusion as Diffusion_PET_DDIPrecon
...
def parse_args_and_config():
    ...
    parser.add_argument("--test_root", type=str, default="./data", help="Directory to test data")
    ...
    # Hyperparameters
    parser.add_argument("--em_itr", type=float, default=1.0, help="Iterations of EM reconstruction")
    parser.add_argument("--em_beta", type=float, default=0.1, help="Beta of HQS")
...
def main():
    ...
    try:
        if "PET" in args.deg:
            runner = Diffusion_PET_DDIPrecon(args, config)
        else:
            print(f"Got {args.deg} as degradation!")
        runner.sample()
    except Exception:
